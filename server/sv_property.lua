...
function Property:new(propertyData)
    ...
    if propertyData.property_id == nil or propertyData.property_id == "nil" then return self end

    local stashName = ("property_%s"):format(propertyData.property_id)
    local stashConfig
    
    -- SM --
    if Config.Shells[propertyData.shell] then
        stashConfig = Config.Shells[propertyData.shell].stash
    else
        stashConfig = SMConfig.data.stash
    end
