...
function RegisterProperty(propertyData, preventEnter, source)
    ...
    if propertyData.propertyType == "smShell" and not propertyData.publicShell then
        local customShellIndex = tonumber(propertyData.customShell) + 1
        local shellData = SMConfig.data.shellSizes[customShellIndex]
        local success, id = exports["smodsk_shellBuilder"]:CreateNewShell(nil, shellData.size)
        propertyData.shellId = id
        propertyData.shell = shellData.locale
    else
        -- So we need to duplicate shell data if we allow players to modify shells --
        if SMConfig.data.allowPlayerModifications then
            local success, id = exports["smodsk_shellBuilder"]:DuplicateShell(propertyData.shellId)
            propertyData.shellId = id
        end
    end

    print(json.encode(propertyData))
    ...
    local cols = "(owner_citizenid, street, region, description, has_access, extra_imgs, furnitures, for_sale, price, shell, shellId, apartment, door_data, garage_data, zone_data)"
    local vals = "(@owner_citizenid, @street, @region, @description, @has_access, @extra_imgs, @furnitures, @for_sale, @price, @shell, @shellId, @apartment, @door_data, @garage_data, @zone_data)"
    ...
    local id = MySQL.insert.await("INSERT INTO properties " .. cols .. " VALUES " .. vals , {
        ...
        ["@shellId"] = propertyData.shellId or '',
